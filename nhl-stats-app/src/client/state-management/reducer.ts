...
export const reducer = (state: AppState, action: AppAction): AppState => {
  switch (action.type) {
    ...
    case ActionTypes.SELECT_SAVED_QUERY:
      ...
      // console.log(state.savedQueries.savedQueries);
      // console.log('setting');
      // console.log(dfToSet);
      return {
        ...state,
    ...
    case ActionTypes.SAVE_QUERY:
      ...
      //   JSON.stringify(action.payload.df.numericFilters)
      // );
      // }, 2000);
      // console.log(
      //   JSON.stringify(state.savedQueries.savedQueries.map((s) => s.df))
      // );
      const jsonData = JSON.stringify(action.payload);
      // const currSavedQueriesArr2 = [...state.savedQueries.savedQueries];
      // console.log(currSavedQueriesArr2.map((x) => x.df.numericFilters));
      // const asddsa = [...currSavedQueriesArr2, action.payload];
      // const l = currSavedQueriesArr2.push(action.payload);
      ...
      return n;
    // console.log(asddsa.map((x) => x.df.numericFilters));
    // const newNumerics: NumericFilterObj[] = [{o}]
    // return {
    //   ...state,
    //   savedQueries: {
    //     savedQueries: asddsa,
    //     selectedIndex: asddsa.length - 1,
    //   },
    // };
